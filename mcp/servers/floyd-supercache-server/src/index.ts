...
import { readFileSync, writeFileSync, existsSync, mkdirSync, statSync, unlinkSync, chmodSync, renameSync } from 'fs';
...
// Cache storage paths
const CACHE_DIR = process.env.FLOYD_SUPERCACHE_DIR || join(homedir(), '.floyd', 'supercache');
...
function getPath(key: string, tier: 'project' | 'reasoning' | 'vault'): string {
  ...
  return join(getTierPath(tier), `${digest}.json`);
}
    ...
    const operation = operationQueue.then(async () => {
      ...
      try {
        ...
        case 'cache_store': {
          ...
          writePrivateJson(filePath, entry);
        ...
        case 'cache_list': {
          ...
          for (const [key, entry] of Object.entries(index.entries)) {
            ...
            // Pattern filter
            if (filter) {
              const regex = wildcardPattern(filter);
        ...
        case 'cache_archive_reasoning': {
          ...
          writePrivateJson(vaultPath, vaultEntry);
